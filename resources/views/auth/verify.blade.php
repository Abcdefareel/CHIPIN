    ...
    <div
        ...
        <div class="bg-white flex flex-col justify-center p-10 sm:p-16 px-6 sm:px-12 min-h-screen">
            <div class="max-w-[420px] w-full mx-auto">
                <h2 class="text-[1.875rem] font-extrabold text-[#1e1b4b] -tracking-[0.02em] mb-1">Verifikasi OTP</h2>
                <p class="text-[0.875rem] text-[#64748b] mb-8">Masukkan email dan kode OTP yang Anda terima.</p>
                ...
                <form method="POST" action="{{ route('password.verify') }}" class="flex flex-col gap-5">
                    ...
                    <div class="flex flex-col gap-2">
                        <label class="text-[0.75rem] font-semibold tracking-wider uppercase text-[#64748b]">OTP</label>
                        <input type="text" name="otp" required
                            class="w-full text-[0.875rem] text-[#1e1b4b] bg-white border border-[#e2e8f0] rounded-2xl p-[0.625rem] px-[0.875rem] focus:border-[#6366f1]" />
                    </div>

                    <button type="submit"
                        class="inline-flex items-center justify-center gap-2 text-[0.875rem] font-semibold leading-none p-[0.875rem] px-6 rounded-2xl bg-gradient-to-r from-indigo-600 to-violet-600 text-white w-full">Verifikasi</button>
                </form>

                <p class="text-center text-[0.875rem] text-[#64748b] mt-6"><a href="{{ route('password.forgot.form') }}"
                        class="font-bold text-[#1e1b4b] hover:underline">Kirim ulang OTP</a></p>
            </div>
        </div>
